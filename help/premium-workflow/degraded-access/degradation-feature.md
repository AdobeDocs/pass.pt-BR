...
>[!IMPORTANT]
>
> A API de degradação é um recurso premium e requer uma licença atual da Adobe.

Ao aplicar uma regra de degradação, os programadores podem ativar temporariamente a autenticação ou autorização automática, garantindo acesso ininterrupto ao conteúdo pelo período em que a degradação for aplicada. As ações de degradação são sempre iniciadas por um programador com base em acordos pré-acordados com os MVPDs. Embora a Adobe não acione diretamente a degradação no momento, os recursos futuros podem incluir o gerenciamento pró-ativo se os SLAs (Service Level Agreements, contratos de nível de serviço) forem estabelecidos.

Este recurso foi projetado para ser usado junto com uma [API de monitoramento de uso](/help/premium-workflow/esm/entitlement-service-monitoring-overview.md) e com base nos contratos prévios com MVPDs, a Autenticação da Adobe Pass oferece uma ferramenta poderosa para equilibrar a experiência do usuário, a confiabilidade e o controle operacional durante momentos críticos.
