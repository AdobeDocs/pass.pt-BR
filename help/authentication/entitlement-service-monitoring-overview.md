...
| sdk-version | A versão do SDK do cliente de autenticação da Adobe Pass |
...
- **Monitoramento** - As equipes de operações ou monitoramento podem criar um painel ou gráfico que chama a API a cada minuto. Usando as informações exibidas, é possível detectar um problema (com a Autenticação do Adobe Pass ou com um MVPD) no minuto em que ele é exibido.
