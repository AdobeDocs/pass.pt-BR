...
A solução de direito de autenticação da Adobe Pass para Android é dividida em dois domínios:
...
1. Iniciar autenticação do Adobe Pass
   ...
   - **Dependência:** Biblioteca Android nativa de autenticação da Adobe Pass (AccessEnabler)

   b. Chame` setRequestor()` para estabelecer a identificação do Programador; passar no `requestorID` e (opcionalmente) uma matriz de endpoints de Autenticação do Adobe Pass.

   - **Dependência:** RequestorID de autenticação do Adobe Pass válido\
...
1. Após um logon bem-sucedido do usuário, a camada do aplicativo detecta o carregamento de um &quot;URL de redirecionamento personalizado&quot; (ou seja: `http://adobepass.android.app`). Na verdade, esse URL personalizado é um URL inválido que não se destina ao carregamento do WebView. É um sinal de que o Fluxo de Autenticação foi concluído e que o WebView precisa ser fechado.
...
1. Valide o token de mídia curta.\
   Use a biblioteca Verificador de token de mídia de autenticação da Adobe Pass para verificar o token de mídia de vida curta retornado do `getAuthorization()` chame acima:
